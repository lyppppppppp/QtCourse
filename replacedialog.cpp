...
void ReplaceDialog::on_btFindNext_clicked()
{
    ...
    if(ui->rbDown->isChecked()){
        ...
        index = text.indexOf(target, c.position(),
                             ui->cbCaseSensetive->isChecked() ? Qt::CaseSensitive : Qt::CaseSensitive);
        if(index >= 0){
            c.setPosition(index);
        ...
        index = text.indexOf(target, c.position() - text.length() - 1,
                             ui->cbCaseSensetive->isChecked() ? Qt::CaseSensitive : Qt::CaseSensitive);
        if(index >= 0){
...
void ReplaceDialog::on_btReplaceAll_clicked()
{
    ...
    if((pTextEdit != nullptr) && (target != "") && (to != "")){
        ...
        text.replace(target, to, ui->cbCaseSensetive->isChecked() ? Qt::CaseSensitive : Qt::CaseSensitive);
